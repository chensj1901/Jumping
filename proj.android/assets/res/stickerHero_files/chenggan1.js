var checkStickLong = {
	...
	run: function() {
		$(".stickMan img").attr({
			'src': 'http://t1.qpic.cn/mblogpic/ffd991e298070e577f0c/2000'
		});
		...
		setTimeout(function() {
			$(".stickMan img").attr({
				'src': 'http://t1.qpic.cn/mblogpic/10d55fe0eec0baa40d1a/2000'
			})
		}, 1000)
	},
	getDown: function() {
	...
	setBestPoint: function() {//｛expires：7｝
//        var a = $.cookie("stickManPoint");
		var a = window.localStorage.getItem('stickManPoint');
		if (!a) {
			a = point;
//            $.cookie("stickManPoint",point,{expires:365*10});
			window.localStorage.setItem('stickManPoint', point)
		} else if (a < point) {
			a = point;
//            $.cookie("stickManPoint",point,{expires:365*10});
			window.localStorage.setItem('stickManPoint', point)
		...
		b.html('我玩到了' + a + '级，世界上只有4个人，能玩到第四十级');
		wx.onMenuShareTimeline({
			title: document.title,
			link: 'http://wxwx.igowo.cn/'+Math.random().toString(36).substr(2,8)+'chenggan/uid=' + Math.random().toString(36).substr(2,8),
			imgUrl: 'http://t1.qpic.cn/mblogpic/647321f31062d08f45d4/2000',
			success: function() {
				window.location.href = 'http://wxwx.igowo.cn/chenggan/chenggan/'
			},
			fail: function() {
				alert('出错')
			},
