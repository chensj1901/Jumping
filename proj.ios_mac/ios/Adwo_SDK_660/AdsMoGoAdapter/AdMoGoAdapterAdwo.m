...
-(void)getAd{
    ...
    switch (type) {
        ...
        case AdViewTypeFullScreen:
            adwo_ad_type = ADWOSDK_FSAD_SHOW_FORM_APPFUN;
...
//    NSLog(@"ration=%@ name=%@",ration,[ration objectForKey:@"nname"]);
    ...
    adView =  nil;
    adView = adwo_ad_type==ADWOSDK_FSAD_SHOW_FORM_APPFUN?AdwoAdGetFullScreenAdHandle(pid,!testMode,self,ADWOSDK_FSAD_SHOW_FORM_APPFUN_WITH_BRAND): AdwoAdCreateBanner(pid,!testMode , self);
    AWSetAGGChannel(adView,ADWOSDK_AGGREGATION_CHANNEL_MOGO);
    MGLog(MGT,@"create adview %@",adView);
...
//            adView.disableGPS = YES;
        ...
        if (adwo_ad_type==ADWOSDK_FSAD_SHOW_FORM_APPFUN) {
            isSuccessful = AdwoAdLoadFullScreenAd(adView,adwo_ad_type,NULL);
        }else{
            isSuccessful
            = AdwoAdLoadBannerAd(adView, adwo_ad_type, NULL);
        }
        if(!isSuccessful)
        {
...
- (void)adwoAdViewDidLoadAd:(UIView*)_adView{
    ...
    if (frame.size.height>110) {
        [[NSNotificationCenter defaultCenter]postNotificationName:@"SJReceiveFullView" object:[_adView autorelease]];
    }
